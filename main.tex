...
%%% Notas da primeira apresentação teste %%%

% 1) No Slide do sumário eu devo apenas ler sem explicar (apresentação)

% 2) No Slide de banco de dados apenas ler a definição exibida (apresentação)

% 3) Apontar com o mouse o que eu falo (apresentação)

% 4) Eu repito muitas vezes o mesmo conceito várias vezes, mesmo no mesmo slide (apresentação)

% 5) Eu levo muito tempo para explicar o slide sobre MongoDB (apresentação)

% 6) A conclusão deve ser mais objetiva e rápida (apresentação)

% 7) Tempo = Contextualização + Problemática + Metodologia + Resultados + Conclusão ~= ~21 + ~6 + ~5 + ~14 + ~2 ~= 43 minutos (informação)

% 8) Lê o meu nome completo na capa (apresentação)

% 9) Não precisa ler as titulações dos orientadores e coorientadores (apresentação)

% 10) Tentar ganhar tempo no slide que explica por que usar o NoSQL (apresentação)

% 11) Não tem problema deixar de explicar algumas funcionalidades, caso seja necessário, por causa do tempo (dica)

% 12) Ler a conclusão ao invés de explicar (apresentação)

% 13) Diminuir a Relação inglês/português (apresentação)

% 14) Slide 7, dizer que a tabela tem um problema, não que está errada (apresentação)

% 15) Diminuir os vício. Ex.: certo? ok? (apresentação)

% 16) Falar com mais calma os pontos fortes da apresentação. Como proposta e exemplos da contribuição. (apresentação)

% 17) Não falar impossível (apresentação)